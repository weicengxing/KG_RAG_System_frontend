...
export function createGameViewComputed(ctx) {
  ...
  const describeLandmark = (landmark) => {
    ...
    if (landmark.type === 'myth_divergence') return own ? withDetail(text(landmark.label, 'Myth divergence'), text(landmark.summary, landmark.winnerLabel, 'Competing meanings')) : 'Other tribe myth divergence'
  ...
  const weatherTemperActions = computed(() => optionMapToList(currentTribe.value?.weatherTemperActions))
  ...
  const newcomerFateActions = computed(() => optionMapToList(currentTribe.value?.newcomerFateActions))
  const observerInterventionActions = computed(() => currentTribe.value?.observerInterventionActions || {})
  ...
  const oralContractOptions = computed(() => optionMapToList(currentTribe.value?.oralContractOptions))
  ...
  const maskPerformanceTypes = computed(() => optionMapToList(currentTribe.value?.maskPerformanceTypes))
  ...
  const migrationEncounterActionOptions = computed(() => optionMapToList(currentTribe.value?.migrationEncounterActions))
