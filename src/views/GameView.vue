...
function triggerPlayerActionAnimation(state = 'gather', player = localPlayer) {
  if (player) {
...
function stanceAnimationForKey(stanceKey = '') {
  return ({
    fire: 'cheer',
    grain: 'sit',
    stone: 'gather',
...
const getGameWsUrl = (token) => {
  ...
  return buildWsUrl(`/ws/game?token=${safeToken}`)
}
